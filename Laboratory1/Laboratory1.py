    ...
    def bisection_method(self,a,b,tr,eps,f,max_iter= 1000):
        ...
        while abs(b-a)>2*eps and i < max_iter:
            ...
            if f(a)*f(c)<0:
                ...
                a = c
        err.append(abs((a+b)/2-tr))
        self.root = float((a+b)/2)
        self.i = i
        self.err = err

    def wegsteins_method(self,x0,tr,eps,f,unfi,max_iter = 1000):
        ...
        while i< max_iter:
            ...
            if unfi(x_next-eps)*unfi(x_next+eps)<0 :
                break
            x_prev = x_n
            x_n = x_next
            xn = xnext
...
class Plotters:
    ...
    def convergence_2(self,a,b,tr,eps,f,phi):
        solver = Solvers()
        solver.wegsteins_method((a+b)/2,tr,eps,phi,f)
        x1 = [i for i in range(solver.i-1)]
        y1 = solver.err
        solver.bisection_method(a,b,tr,eps,f)
        x2 = [i for i in range(solver.i+1)]
        y2 = solver.err
    ...
    def initial_guess_effect(self,method,a,b,eps,fi,f,tr=-1000):
        solver = Solvers()
        x_list = np.linspace(a,b,100)
        i_list = []
        if method == 'Wegstein':
            for x in x_list:
                solver.wegsteins_method(x,tr,eps,fi,f)
                i_list.append(solver.i)
        else:
            print('Choose a valid method name')
        y = i_list
        ...
        plt.plot(x_list,y) 
        if tr != -1000:
            plt.axvline(tr,linestyle = 'dashed')
            yy = plt.ylim()
            plt.annotate(f'$x^*$ = {tr}',[tr,min(yy)],[tr,min(yy)-0.2])
        plt.grid(True)
        plt.xlabel('$x_0$')
        plt.ylabel('Iterations')
        plt.show()
...
solver1.wegsteins_method(1.5,0,1e-6,algebraic_fi,algebraic)
print(round(solver1.root,7))
solver2.bisection_method(0.4,1,0,1e-6,trans)
print(round(solver2.root,7))
solver2.wegsteins_method(1.5,0,1e-6,trans_fi,trans)
print(round(solver2.root,7))
# ###Algebraic
...
plotter.initial_guess_effect('Wegstein',0.4,1,1e-6,trans_fi,trans,tr = 0.6115458)
plotter.initial_guess_effect('Wegstein',1,2,1e-6,algebraic_fi,algebraic,tr = 1.6180334)