...
class Integrator:
    ...
    def radau_3(self, eps):
        ...
        while i < self.max_iter:
            ...
            result_new = np.sum(
                [self._s_radau_3(intervals[j], intervals[j+1]) for j in range(N)])
            calls += N*3
            if self.runge_rule(result_prev, result_new) < eps:
                return result_new, calls
            result_prev = result_new
        raise RuntimeError(
            f"{self.radau_3.__name__} did not achieve desired accuracy after {i} iterations. Got: {result_new}")

    def exact_val(self):
        return quad(self.f, self.a, self.b, epsabs=1e-18)[0]
