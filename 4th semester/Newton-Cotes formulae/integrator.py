...
class Integrator:
    ...
    # def N_C_n(self,n, eps): ERRORED, remove H from _s_n()
    #     H = np.array(self.cotes_coefs(n))
    #     blocks = 1 
    #     i = 0
    #     S_prev = self._s_n(n,H,self.a, self.b)
        
    #     while i < self.max_iter:
    #         i += 1
    #         blocks *= 2
    #         split = np.linspace(self.a, self.b, blocks + 1)
            
    #         S_new = sum(self._s_n(n,H,split[j], split[j+1]) for j in range(blocks))
            
    #         if self.runge_rule(S_prev, S_new, p=n) < eps:
    #             return S_new, i
            
    #         S_prev = S_new
    #     raise RuntimeError(f"Method neight did not converge. Blocks: {blocks}")
    @staticmethod
    def cotes_coefs(num):
...
# def f(x):
#     # Имитируем сложный расчет для каждого элемента массива x
#     # Например, считаем 500 итераций какой-нибудь последовательности
#     res = np.zeros_like(x)
#     for _ in range(500):
#         res = np.sin(x + res) + np.cos(x - res)
#     return res
# integrator = Integrator(f,2,10)
# print(integrator.cotes_coefs(9))
# t1 = time.perf_counter()
# S1,i1 = integrator.trapz_efficent(1e-10)
# t2 = time.perf_counter()
# S2,i2 = integrator.trapz_naive(1e-10)
# t3 = time.perf_counter()
# print("The results:")
# print(f"Naive: I = {S2},iter = {i2}, T = {t3-t2}")
# print(f"Efficent: I = {S1},iter ={i1}, T = {t2-t1}")
# print(f"Difference: efficient is {(t3-t2)/(t2-t1)} times faster than naive, deviation in results = {abs(S2-S1)}")
